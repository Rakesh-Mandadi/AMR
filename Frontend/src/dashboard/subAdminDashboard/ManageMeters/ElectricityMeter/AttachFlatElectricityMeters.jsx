...
const AttachFlatElectricityMeters = () => {
  ...
  */
  ...
  useEffect(() => {
    const fetchBuildings = async () => {
     // if (!facilityDetails.facilityId) return;
      ...
      try {
        const tokenD = localStorage.getItem('token');
       // console.log("FD",facilityDetails)
  ...
  const handleAddMeterClick = async () => {
    try {
      const tokenD = localStorage.getItem('token');
      const response = await axios.get(`${config.backendurl}/api/v1/users/${facilityId}/E/unassigned-status`, {
        headers: {
          'Authorization': `Bearer ${tokenD}`,
          'Content-Type': 'application/json',
        }
  ...
  export default AttachFlatElectricityMeters;
