...
const AddNewGasMeters = () => {
  ...
  useEffect(() => {
    ...
    const fetchMeterTypes = async () => {
      try {
        const tokenD = localStorage.getItem('token');
        const response = await axios.get(`${config.backendurl}/api/v1/users/${facilityDetails.facilityId}/G`, {
          headers: {
            'Authorization': `Bearer ${tokenD}`,
            'Content-Type': 'application/json',
          }
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      const tokenD = localStorage.getItem('token');
      const response = await axios.post(`${config.backendurl}/api/v1/users/${facilityDetails.facilityId}/addmeter`, formData, {
        headers: {
          Authorization: `Bearer ${tokenD}`,
          'Content-Type': 'application/json',
        },
...
export default AddNewGasMeters;
