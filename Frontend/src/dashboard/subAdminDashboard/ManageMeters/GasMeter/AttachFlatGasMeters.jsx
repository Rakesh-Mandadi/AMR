...
const AttachFlatGasMeters = () => {
  ...
  */
  ...
  const handleAddMeterClick = async () => {
    try {
      const tokenD = localStorage.getItem('token');
      const response = await axios.get(`${config.backendurl}/api/v1/users/${facilityId}/G/unassigned-status`, {
        headers: {
          'Authorization': `Bearer ${tokenD}`,
          'Content-Type': 'application/json',
        }
  ...
  export default AttachFlatGasMeters;
